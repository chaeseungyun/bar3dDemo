...
export class Bar3DRenderer {
  ...
  _buildAxes() {
    ...
    // 제목도 카메라를 향하는 빌보드(Sprite) → 회전 없이 항상 읽힘.
    ...
    // amplitude 제목만 화면상 세로로 읽히도록 90° 회전 (카메라는 계속 향함).
    this._ampTitle.material.rotation = Math.PI / 2;
  ...
  _updateOrientation() {
    ...
    // 눈금은 앞쪽/바깥쪽 모서리로, 제목은 그보다 더 바깥으로 배치 (전부 카메라 향함).
